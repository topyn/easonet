...
export default function BrandPageView() {
  ...
  async function joinWaitlist() {
    ...
    await fetch('/api/waitlists/join', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 60 }}>
            {(page.links as Link[]).map((link, i) => (
              <a key={i} href={link.url} target="_blank" rel="noreferrer"
                style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '16px 20px', background: bg2, border: `1px solid ${border}`, borderRadius: 12, transition: 'border-color .2s', cursor: 'pointer' }}
