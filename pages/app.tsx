...
interface DnsResult { mx: boolean; spf: boolean; mxRecord?: string; spfRecord?: string }
...
function DnsWizard({ identity, onVerified }: { identity: Identity; onVerified: () => void }) {
  ...
  const s: Record<string, any> = {
    ...
    recordLabel: { fontSize: 11, color: '#888', textTransform: 'uppercase' as const, letterSpacing: '.05em' },
    copyRow: { display: 'flex', alignItems: 'center', gap: 8 },
    copyVal: { flex: 1, color: '#1a1a1a', wordBreak: 'break-all' as const },
    copyBtn: { fontSize: 11, padding: '3px 8px', border: '0.5px solid #ccc', borderRadius: 5, cursor: 'pointer', background: '#fff', whiteSpace: 'nowrap' as const },
    ...
    btn: { padding: '10px 20px', background: '#534AB7', color: '#fff', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer', alignSelf: 'flex-start' as const },
  ...
  if (step === 'done') return (
    <div style={s.wrap}>
      <div style={s.successBox}>DNS verified — {identity.email} is ready to use</div>
    </div>
  )
  ...
  return (
    <div style={s.wrap}>
      ...
      <div style={s.sub}>Add these two records in your DNS provider, then click Check. It can take a few minutes to propagate.</div>
      ...
      {step === 'checking' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div style={s.checkRow}>
            <div style={s.tick(dns?.mx ?? null)}>{dns?.mx ? '✓' : dns ? '✗' : '…'}</div>
            <span style={{ color: dns?.mx ? '#0F6E56' : '#666' }}>MX record {dns?.mx ? 'found' : 'checking…'}</span>
          </div>
          <div style={s.checkRow}>
            <div style={s.tick(dns?.spf ?? null)}>{dns?.spf ? '✓' : dns ? '✗' : '…'}</div>
            <span style={{ color: dns?.spf ? '#0F6E56' : '#666' }}>SPF record {dns?.spf ? 'found' : 'checking…'}</span>
          </div>
          <div style={{ fontSize: 12, color: '#aaa' }}>Checking every 4 seconds — DNS can take up to 10 minutes</div>
        </div>
      )}
...
export default function App() {
  ...
  useEffect(() => { if (!loading) { loadIdentities(); loadThreads() } }, [loading, loadIdentities, loadThreads])
  ...
  async function sendEmail() {
    ...
    await post('/api/emails/send', {
      identityId: composeIdentityId, to: composeTo,
      subject: composeSubject, text: composeText,
    })
  ...
  const s: Record<string, any> = {
    ...
    sidebarScroll: { flex: 1, overflowY: 'auto' as const, padding: '12px 0' },
    sidebarSection: { padding: '0 12px', marginBottom: 16 },
    sidebarLabel: { fontSize: 11, color: '#aaa', textTransform: 'uppercase' as const, letterSpacing: '.06em', marginBottom: 6, padding: '0 4px' },
    ...
    iName: { fontSize: 13, fontWeight: 500, whiteSpace: 'nowrap' as const, overflow: 'hidden', textOverflow: 'ellipsis', color: '#1a1a1a' },
    iEmail: { fontSize: 11, color: '#999', whiteSpace: 'nowrap' as const, overflow: 'hidden', textOverflow: 'ellipsis' },
    ...
    addBtn: { width: '100%', textAlign: 'left' as const, padding: '7px 8px', fontSize: 12, color: '#888', border: 'none', background: 'none', cursor: 'pointer', borderRadius: 8 },
    ...
    threadList: { flex: 1, overflowY: 'auto' as const },
    ...
    threadTime: { fontSize: 11, color: '#bbb', whiteSpace: 'nowrap' as const },
    threadSubject: (unread: boolean) => ({ fontSize: 13, fontWeight: unread ? 500 : 400, color: '#333', whiteSpace: 'nowrap' as const, overflow: 'hidden', textOverflow: 'ellipsis' }),
    threadPreview: { fontSize: 12, color: '#aaa', whiteSpace: 'nowrap' as const, overflow: 'hidden', textOverflow: 'ellipsis', marginTop: 1 },
    ...
    empty: { flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#bbb', fontSize: 14, flexDirection: 'column' as const, gap: 8 },
    ...
    messages: { flex: 1, overflowY: 'auto' as const, padding: '20px', display: 'flex', flexDirection: 'column', gap: 16 },
    ...
    msgBubble: (out: boolean) => ({ maxWidth: '72%', background: out ? '#534AB7' : '#f5f5f3', color: out ? '#fff' : '#1a1a1a', padding: '10px 14px', borderRadius: out ? '14px 14px 4px 14px' : '14px 14px 14px 4px', fontSize: 14, lineHeight: 1.55, whiteSpace: 'pre-wrap' as const }),
    composePanel: { position: 'absolute' as const, bottom: 0, right: 0, width: 480, background: '#fff', border: '0.5px solid #ccc', borderRadius: '12px 12px 0 0', display: 'flex', flexDirection: 'column', boxShadow: '0 -4px 20px rgba(0,0,0,.08)', zIndex: 10 },
    ...
    bodyInput: { width: '100%', border: 'none', outline: 'none', fontSize: 13, resize: 'none' as const, minHeight: 120, background: 'transparent', color: '#1a1a1a', fontFamily: 'system-ui,sans-serif' },
    ...
    modal: { position: 'fixed' as const, inset: 0, background: 'rgba(0,0,0,.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 30 },
    modalBox: { background: '#fff', borderRadius: 14, padding: 28, width: 400, display: 'flex', flexDirection: 'column', gap: 14, maxHeight: '90vh', overflowY: 'auto' as const },
    modalTitle: { fontSize: 16, fontWeight: 600 },
    input: { border: '0.5px solid #d0cdc6', borderRadius: 8, padding: '9px 12px', fontSize: 13, outline: 'none', width: '100%', boxSizing: 'border-box' as const },
    colorRow: { display: 'flex', gap: 8, flexWrap: 'wrap' as const },
    colorSwatch: (color: string, selected: boolean) => ({ width: 26, height: 26, borderRadius: '50%', background: color, cursor: 'pointer', border: selected ? `2px solid #1a1a1a` : '2px solid transparent', boxSizing: 'border-box' as const }),
  ...
  const visibleThreads = activeIdentityId
    ? threads.filter(t => t.identity?.id === activeIdentityId)
    : threads
        ...
        <div style={s.sidebar}>
          <div style={s.sidebarScroll}>
            <div style={s.sidebarSection}>
              ...
              {identities.map(id => (
                <div key={id.id} style={s.identityRow(activeIdentityId === id.id, id.color)}
                  ...
                  {!id.dnsVerified && (
                    <div title="DNS not verified" style={s.unverifiedDot} onClick={e => { e.stopPropagation(); setWizardIdentity(id) }} />
                  )}
                </div>
              ))}
        ...
        <div style={s.main}>
          ...
          {wizardIdentity && (
            <div style={{ flex: 1, overflowY: 'auto' as const }}>
              <div style={{ ...s.toolbar }}>
                <button style={{ ...s.backBtn, fontSize: 13 }} onClick={() => setWizardIdentity(null)}>← Back</button>
              ...
              {activeThread ? (
                ...
                <div style={s.threadDetail}>
                  ...
                  <div style={s.messages}>
                    {(activeThread.messages ?? []).map(m => (
                      <div key={m.id} style={s.msgWrap(m.direction === 'outbound')}>
                        <div style={s.msgMeta}>
                          {m.direction === 'outbound' ? `You (${m.fromAddress})` : m.fromAddress}
                  ...
                  <div style={{ padding: '12px 20px', borderTop: '0.5px solid #e0ddd6', display: 'flex', gap: 8 }}>
                    <textarea
                      style={{ flex: 1, border: '0.5px solid #e0ddd6', borderRadius: 8, padding: '8px 12px', fontSize: 13, resize: 'none' as const, outline: 'none', height: 60, fontFamily: 'system-ui' }}
                ...
                <div style={s.threadList}>
                  {visibleThreads.length === 0 ? (
                    ...
                    const otherParty = lastMsg?.direction === 'inbound'
                      ? lastMsg.fromAddress.replace(/<.*>/, '').trim()
                      : (t.participants?.find(p => p !== t.identity?.email) ?? 'You')
                      ...
                      <div key={t.id} style={s.thread(isUnread)} onClick={() => openThread(t)}>
                        ...
                        <div style={s.threadBody}>
                          <div style={s.threadRow}>
                            <span style={s.threadFrom(isUnread)}>
                              {otherParty}
                              <span style={s.pill(t.identity?.color ?? '#888')}>{t.identity?.name}</span>
                            </span>
        ...
        <div style={s.modal} onClick={e => e.target === e.currentTarget && setAddingIdentity(false)}>
          <div style={s.modalBox}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button style={s.modalBtn} onClick={addIdentity} disabled={user?.plan === 'trial' && identities.length >= 3}>
                Add identity
              </button>
